...
+ Atomic data saves (prevents corruption)
+ High DPI support
+ XDG-compliant paths (~/.config/watchdrift/)
...
ctk.set_widget_scaling(1.0)  # Adjust to 1.1 or 1.2 if blurry on 4K

# ===== XDG-COMPLIANT PATHS =====
CONFIG_DIR = Path.home() / ".config" / "watchdrift"
CONFIG_DIR.mkdir(parents=True, exist_ok=True)
DATA_FILE = CONFIG_DIR / "watch_data.json"

def load_data():
    """Load watch recordings from disk."""
    if DATA_FILE.exists():
        try:
...
def save_data(data):
    """
    Safely save data using atomic write pattern.
    This prevents data corruption if the app crashes during save.
    """
    try:
...
def sync_ntp():
    """Synchronize with NTP server (runs in background thread)."""
    global ntp_offset
    if not ntplib:
        return False
...
class WatchDriftApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        ctk.CTkLabel(sidebar, text="WATCHES", 
                     font=ctk.CTkFont(size=18, weight="bold")).pack(pady=20)

        self.watch_list = ctk.CTkScrollableFrame(sidebar)
        self.watch_list.pack(fill="both", expand=True, padx=15, pady=10)

        ctk.CTkButton(sidebar, text="+ Add New Watch", 
                      command=self.add_watch).pack(pady=10, padx=20)
    ...
    def force_ntp_sync(self):
        """Sync PC time to NTP (non-blocking)."""
        self.pc_drift_label.configure(text="Syncing...", text_color="yellow")
        
        def run_sync():
            success = sync_ntp()
            self.after(0, lambda: self.on_sync_complete(success))
        
        threading.Thread(target=run_sync, daemon=True).start()

    def on_sync_complete(self, success):
        """Handle NTP sync result."""
        if success:
            ...
            messagebox.showwarning("NTP Sync", "⚠️ Could not reach NTP server.\nUsing system time.")
        self.update_pc_drift()

    def update_pc_drift(self):
        """Update PC drift display."""
    ...
    def delete_watch(self, wid):
        """Delete a watch and all its recordings."""
    ...
    def build_record_tab(self):
        """Build the Record tab."""
        tab = self.tabview.tab("Record")
        for widget in tab.winfo_children():
            widget.destroy()

        if not self.selected_id:
            ctk.CTkLabel(tab, text="Select a watch to begin recording",
                        font=ctk.CTkFont(size=14)).pack(pady=100)
            return

        ctk.CTkLabel(tab, text="Live NTP Time", 
                     font=ctk.CTkFont(size=14)).pack(pady=5)
        
        self.clock_label = ctk.CTkLabel(tab, text="--:--:--",
        ...
        ctk.CTkCheckBox(tab, text="Show 1-second progress bar (helps sync)",
                        ...
                        command=self.toggle_progress_bar).pack(pady=8)

        self.progress_canvas = ctk.CTkCanvas(tab, height=30, bg="#1a1a1a",
        ...
        self.status_label = ctk.CTkLabel(tab, text="Press the button above",
        ...
        ctk.CTkButton(frame, text="−1 min", width=100,
                      command=self.minus_min).pack(side="left", padx=10)
        
        self.adj_label = ctk.CTkLabel(frame, text="0 min",
                                      font=ctk.CTkFont(size=18))
        self.adj_label.pack(side="left", padx=30)
        
        ctk.CTkButton(frame, text="+1 min", width=100,
                      command=self.plus_min).pack(side="left", padx=10)
    ...
    def record_drift(self):
        ...
        if is_first:
            total = 0
            msg = "✓ FIRST RECORDING → 0 drift baseline"
            self.minute_adjust = 0
        else:
        ...
        reading = {
            "id": str(int(t_ms)),
            "watchId": self.selected_id,
            "ts": t_ms,
            "ntpSeconds": ntp_s,
            "minuteAdjust": self.minute_adjust,
    ...
    def build_log_tab(self):
        ...
        ctk.CTkLabel(tab, text=f"{w['name']} — {len(rs)} recordings",
                     font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        
        ctk.CTkButton(tab, text="🗑 Clear Entire Log", fg_color="#c06060",
    ...
    def clear_log(self):
        """Clear all recordings for selected watch."""
    ...
    def build_trend_tab(self):
        """Build the Trend tab with graph."""
        ...
        rs = sorted([r for r in self.data["readings"] if r["watchId"] == self.selected_id],
                   key=lambda r: r["ts"])
        ...
        ctk.CTkLabel(summary, text="SUMMARY",
                     font=ctk.CTkFont(size=14, weight="bold")).pack(pady=5)
        ctk.CTkLabel(summary, text=f"Accumulative Drift:   {fmt_deviation(total_drift)}",
                     font=ctk.CTkFont(size=18)).pack(pady=2)
        ctk.CTkLabel(summary, text=f"Daily Drift Rate:      {fmt_deviation(rate) if rate else '—'}",
                     font=ctk.CTkFont(size=18)).pack(pady=2)
        ctk.CTkLabel(summary, text=f"Measured over:         {days:.1f} days",
        ...
        canvas.create_text(50 + w, h + 45, text="latest", fill="#888",
